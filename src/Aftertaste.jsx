...
/**
 * Aftertaste — Tableside QR Feedback & Review Generation
 * Single-file, mobile-first React + Tailwind (core utilities only).
 * No required props. Default export.
 */

/* ============================================================
   ⚙️  EDIT ME — Restaurant settings
   Everything a manager needs to customize lives here.
   No need to touch the component code below.
   ============================================================ */
const CONFIG = {
  restaurantName: "Aftertaste",
  tagline: "Guest Feedback",

  // The table shown when a guest first scans the QR code.
  defaultTable: "Table 4",

  // Your tables / sections. Add, remove, or rename freely.
  tables: [
    "Table 1", "Table 2", "Table 4", "Table 7", "Table 9",
    "Table 12", "Table 15", "Table 18", "Table 21", "Patio 3", "Bar 2",
  ],

  // Your servers. Add, remove, or rename freely.
  servers: ["Ava M.", "Marco R.", "Priya S.", "Daniel K.", "Sofia L."],

  // Paste your Google review link here (from your Google Business profile).
  googleReviewUrl: "https://search.google.com/local/writereview?placeid=YOUR_PLACE_ID",

  // The perk happy guests get for leaving a review.
  reward: "free coffee or dessert",
};
...
export default function Aftertaste() {
  const [table, setTable] = useState(CONFIG.defaultTable);
  const [rating, setRating] = useState(0);
  const [hover, setHover] = useState(0);
  const [step, setStep] = useState("rating"); // rating | positive | negative | success
  const [successKind, setSuccessKind] = useState("private"); // private | google

  // negative-path state
  const [issues, setIssues] = useState([]);
  const [comment, setComment] = useState("");

  // staff rating state (shared by both paths)
  const [server, setServer] = useState("");
  const [serverVote, setServerVote] = useState(null); // "up" | "down" | null

  const isPositive = rating >= 4;

  function chooseRating(n) {
    setRating(n);
    // brief pause so the star fill animation reads before routing
    window.setTimeout(() => {
      setStep(n >= 4 ? "positive" : "negative");
    }, 520);
  }
  ...
  function goGoogle() {
    ...
    setStep("success");
    // Open the real review page in a new tab (harmless placeholder in preview).
  ...
  return (
    <div className="min-h-screen w-full bg-neutral-950 flex justify-center font-sans text-neutral-100 antialiased selection:bg-amber-400/30">
      ...
      {/* Phone frame — feels premium on desktop, edge-to-edge on mobile */}
      <div className="relative w-full max-w-[430px] min-h-screen bg-gradient-to-b from-neutral-900 via-neutral-950 to-black overflow-hidden shadow-2xl">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="leading-tight">
                <p className="text-[10px] uppercase tracking-[0.28em] text-amber-400/90">
                  {CONFIG.tagline}
                </p>
                <h1 className="font-serif text-xl font-semibold tracking-wide text-neutral-50">
        ...
        <main className="relative z-10 px-6 pb-16">
          ...
          {step === "success" && (
            <SuccessStep kind={successKind} table={table} onDone={resetAll} />
          )}
        </main>
...
function TableSelector({ table, setTable }) {
  return (
            ...
            >
              <Star
                className={[
                  ...
                  on
                    ? "scale-110 text-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.55)]"
      ...
      <div className="mt-5 h-6 text-center">
        <span
          ...
          className={[
            ...
            active ? "animate-rise text-amber-300" : "text-transparent",
          ].join(" ")}
        >
        ...
        {[1, 2, 3, 4, 5].map((n) => (
          <span
            ...
            className={[
              ...
              n <= active ? "w-6 bg-amber-400" : "w-1.5 bg-neutral-700",
            ].join(" ")}
          />
        ))}
      </div>
...
function BackButton({ onBack }) {
  return (
    <button
      type="button"
      onClick={onBack}
      className="mb-5 inline-flex items-center gap-1.5 text-xs font-medium text-neutral-400 transition hover:text-amber-300"
    >
  ...
  return (
    <div className="inline-flex items-center gap-1 rounded-full bg-white/[0.05] px-2.5 py-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          className={n <= rating ? "h-3.5 w-3.5 text-amber-400" : "h-3.5 w-3.5 text-neutral-700"}
...
function PositiveStep({ rating, onBack, onGoogle, server, setServer, serverVote, setServerVote }) {
  return (
    <section className="animate-rise pt-2">
      ...
      <div className="rounded-3xl border border-amber-400/20 bg-gradient-to-b from-amber-400/[0.08] to-transparent p-6 text-center">
        ...
        <h2 className="mt-4 font-serif text-2xl leading-snug text-neutral-50">
          We're thrilled you enjoyed your meal!
        </h2>
        <p className="mx-auto mt-2 max-w-[17rem] text-sm text-neutral-300">
          Would you share the love? A quick Google review means the world to our
          small team.
        </p>
        ...
        <div className="mt-5 flex items-center justify-center gap-2 rounded-2xl border border-amber-400/25 bg-amber-400/[0.06] px-4 py-3">
          ...
          <p className="text-left text-xs text-amber-100">
            Show your review to your server for a{" "}
            <span className="font-semibold text-amber-300">{CONFIG.reward}</span>.
          </p>
        </div>
        ...
        <p className="mt-3 text-[11px] text-neutral-500">
          Opens Google · takes about 20 seconds
        </p>
      </div>
  ...
  return (
    <section className="animate-rise pt-2">
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="font-serif text-xl leading-snug text-neutral-50">
              We're sorry we missed the mark.
            </h2>
            <p className="mt-1 text-sm text-neutral-400">
              Please help us fix it — this stays private with our owners.
            </p>
          </div>
        </div>
              ...
              >
                <span
                  className={[
                    ...
                    on ? "bg-amber-400 text-neutral-900" : "bg-white/[0.06] text-neutral-400",
                  ].join(" ")}
                >
      ...
      <p className="mt-3 text-center text-[11px] text-neutral-500">
        Only the owners see this — it won't be posted publicly.
      </p>
    </section>
  );
}

function StaffRating({ server, setServer, serverVote, setServerVote, tone }) {
  return (
    <div className="mt-4 rounded-3xl border border-white/10 bg-white/[0.02] p-5">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-neutral-200">
          Rate your server
          <span className="ml-2 text-[11px] font-normal text-neutral-500">optional</span>
        </p>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2.5">
        ...
        <button
          ...
          onClick={() => setServerVote(serverVote === "up" ? null : "up")}
          className={[
            "grid h-12 w-12 place-items-center rounded-xl border transition-all duration-200 active:scale-90",
            serverVote === "up"
        ...
        <button
          ...
          onClick={() => setServerVote(serverVote === "down" ? null : "down")}
          className={[
            "grid h-12 w-12 place-items-center rounded-xl border transition-all duration-200 active:scale-90",
            serverVote === "down"
...
function SuccessStep({ kind, table, onDone }) {
    ...
    <section className="animate-rise flex min-h-[70vh] flex-col items-center justify-center pt-6 text-center">
      ...
      <div className="relative">
        ...
        <div className="relative grid h-24 w-24 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 shadow-2xl shadow-amber-900/40 animate-pop">
          {google ? (
            <Coffee className="h-11 w-11 text-neutral-900" strokeWidth={2.2} />
          ) : (
            <Check className="h-12 w-12 text-neutral-900" strokeWidth={3} />
          )}
        </div>
      </div>

      <h2 className="mt-7 font-serif text-3xl text-neutral-50">
        {google ? "Thank you!" : "Feedback received"}
      </h2>
      <p className="mx-auto mt-3 max-w-[17rem] text-sm text-neutral-300">
        {google
          ? `You're being taken to Google. Don't forget to show your review for a ${CONFIG.reward}!`
      ...
      <p className="mt-3 text-[11px] text-neutral-600">
        Resetting for the next guest in {count}s
      </p>
    </section>
  );
}
...
function Confetti() {
  const pieces = useRef(
    Array.from({ length: 16 }).map((_, i) => ({
      ...
      color: ["#fbbf24", "#f59e0b", "#fb923c", "#fef3c7", "#f43f5e"][i % 5],
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      {pieces.map((p, i) => (
        <span
          key={i}
          className="absolute top-0 block rounded-sm animate-confetti"
          style={{
            left: `${p.left}%`,
            width: p.size,
            height: p.size * 0.5,
